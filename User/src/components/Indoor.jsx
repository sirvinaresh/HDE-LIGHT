...
function Indoor() {
  ...
  },
  {
    ...
    localStorage.setItem('activeindoorcat',JSON.stringify({catSlug}));
    setactive(catSlug)
    ...
    const getSubCategory = (catSlug, scatSlug) => {
        localStorage.setItem('activeindoorcat',JSON.stringify({catSlug,scatSlug}));
        setactive(scatSlug)
    ...
    useEffect(()=>{
        const saved = JSON.parse(localStorage.getItem('activeindoorcat')) || {catSlug:'led-tube-light'};
